...
const template = (componentName, importPath) => `"use client";
...
const styles = [
  { dir: 'style-1', comp: 'CinematicHorizontalSnap', file: '@/components/shared/cinematic-horizontal-snap' },
  { dir: 'style-2', comp: 'FocusExpansionCarousel', file: '@/components/shared/focus-expansion-carousel' },
  { dir: 'style-3', comp: 'HeroCrossfade', file: '@/components/shared/hero-crossfade' },
  { dir: 'style-4', comp: 'ScaleCoverflow2D', file: '@/components/shared/scale-coverflow-2d' },
];

styles.forEach(s => {
  const dir = path.join(__dirname, 'app', '(public)', 'gallery', s.dir);
  fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(path.join(dir, 'page.tsx'), template(s.comp, s.file));
});
