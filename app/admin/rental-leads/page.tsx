...
export default function AdminRentalLeadsPage() {
  ...
  const loadLeads = async (showSpinner = true) => {
    ...
    try {
      ...
      // Merge local CRM storage leads for all Rentals
      try {
        const local = localStorage.getItem("user_uploaded_crm_leads");
        if (local) {
          const parsed = JSON.parse(local);
          if (Array.isArray(parsed)) {
            const matching = parsed.map((l: any) => ({
              ...
              returnDateTime: null,
              tripType: l.tripType || "Car Rental",
              notes: l.notes || "",
              status: (l.status || "NEW") as LeadStatus,
              vehicleCategoryId: "default",
              vehicleCategory: { name: "Selected Vehicle" },
              createdAt: l.createdAt || new Date().toISOString()
            }));

            const map = new Map<string, RentalLead>();
    ...
    <div className="bg-background min-h-screen text-slate-100 p-8 space-y-8">
      ...
      <div className="glassmorphism p-6 rounded-xl border border-border flex flex-col md:flex-row gap-4 items-center justify-between">
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
          ...
          >
            <option value="" className="bg-surface">All Rental Types</option>
            <option value="LOCAL" className="bg-surface">Local</option>
            <option value="OUTSTATION" className="bg-surface">Outstation</option>
            <option value="AIRPORT" className="bg-surface">Airport</option>
            <option value="HOURLY" className="bg-surface">Hourly</option>
            <option value="ROUND_TRIP" className="bg-surface">Round Trip</option>
            <option value="ONE_WAY" className="bg-surface">One Way</option>
          </select>
