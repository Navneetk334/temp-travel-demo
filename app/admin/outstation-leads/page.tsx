...
export default function AdminOutstationLeadsPage() {
  ...
  const loadLeads = async (showSpinner = true) => {
    ...
    try {
      ...
      // Merge local CRM storage leads for Outstation
      try {
        ...
        if (local) {
          ...
          if (Array.isArray(parsed)) {
            const matching = parsed.filter((l: any) => 
              (l.tripType || "").toLowerCase().includes("outstation") || 
              (l.tripType || "").toLowerCase().includes("round trip") ||
              (l.tripType || "").toLowerCase().includes("one way") ||
              (l.notes || "").toLowerCase().includes("outstation")
            ).map((l: any) => ({
              ...
              dropLocation: l.dropLocation || "",
              pickupDateTime: l.createdAt || new Date().toISOString(),
              returnDateTime: null,
              tripType: l.tripType || "Outstation Trip",
              notes: l.notes || "",
      ...
      console.error("Failed to load outstation leads:", err);
    } finally {
  ...
  const handleAddNoteLog = async (id: string) => {
    ...
    try {
      ...
      const res = await fetch(`/api/rental/lead/${id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ notes: existingNotes }),
      });

      if (res.ok) {
        setLeads(leads.map((l) => (l.id === id ? { ...l, notes: existingNotes } : l)));
        if (activeLead && activeLead.id === id) {
          setActiveLead({ ...activeLead, notes: existingNotes });
        }
        setNewNoteInput("");
      }
    } catch (err) {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Are you sure you want to permanently delete this outstation lead?")) return;
    ...
    try {
      await fetch(`/api/rental/lead/${id}`, { method: "DELETE" });
      loadLeads(false);
    } catch (e) {
  ...
  const handleExportCSV = () => {
    ...
    const headers = ["ID", "Customer Name", "Email", "Phone", "Trip Type", "Pickup Address", "Drop Address", "Pickup Date/Time", "Status", "Created At"];
    const rows = leads.map((l) => [
      l.id,
      `"${l.customerName}"`,
      `"${l.email}"`,
      `"${l.phone}"`,
      `"${l.tripType || "Outstation"}"`,
      `"${l.pickupLocation}"`,
      `"${l.dropLocation || ""}"`,
    ...
    link.setAttribute("download", `outstation_leads_${new Date().toISOString().slice(0, 10)}.csv`);
    ...
    <div className="bg-slate-950 min-h-screen text-slate-100 p-8 space-y-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-white/5 pb-6 gap-4">
        <div>
          <h1 className="text-3xl font-extrabold text-slate-50 tracking-tight flex items-center gap-2.5">
            ...
            <span>Outstation Leads CRM</span>
          </h1>
          <p className="text-xs text-slate-400 mt-1">
            Manage One-Way and Round-Trip intercity highway travel inquiries, status pipeline, sales call logs, and data exports.
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-7 glassmorphism rounded-xl border border-white/5 overflow-hidden flex flex-col space-y-2 p-2 sm:p-0">
          ...
          {loading ? (
            <div className="text-center py-16 text-slate-400 text-xs">Loading outstation leads...</div>
          ) : (
            <>
                    ...
                    {leads.length === 0 ? (
                      <tr>
                        <td colSpan={6} className="text-center py-16 text-slate-500 italic">
                          No outstation leads matching criteria found.
                        </td>
                      </tr>
                    ) : (
                      leads.map((lead, idx) => (
                        ...
                        >
                          ...
                          <td className="p-4">
                            ...
                            <div className="text-slate-400 mt-0.5"><span className="font-mono text-slate-400">{lead.phone}</span> &bull; {lead.email}</div>
                          </td>
                          <td className="p-4 font-mono text-slate-300 max-w-xs truncate">
                            {lead.pickupLocation} → {lead.dropLocation || "N/A"}
                          </td>
                          <td className="p-4">
            ...
            <div className="bg-slate-900 border border-white/5 p-6 rounded-2xl space-y-6 glassmorphism">
              ...
              <div className="space-y-3 border-t border-white/5 pt-4 text-xs text-slate-300">
                ...
                <div className="flex justify-between items-center bg-slate-950/40 p-2.5 rounded-lg border border-white/5">
                  ...
                  <span className="font-bold text-amber-400">{activeLead.tripType || "Outstation"}</span>
                </div>
                <div className="flex justify-between items-center bg-slate-950/40 p-2.5 rounded-lg border border-white/5">
                ...
                <div className="bg-slate-950/40 p-3 rounded-lg border border-white/5 space-y-1">
                  <div className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Drop Address</div>
                  <div className="text-slate-200 leading-relaxed">{activeLead.dropLocation || "N/A"}</div>
                </div>
              </div>
