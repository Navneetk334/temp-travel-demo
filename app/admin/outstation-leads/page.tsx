...
export default function AdminOutstationLeadsPage() {
  ...
  const handleExportCSV = () => {
    ...
    const headers = ["ID", "Customer Name", "Email", "Phone", "Trip Type", "Pickup", "Drop", "Pickup Date", "Status"];
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-900/60 p-6 rounded-2xl border border-white/5 backdrop-blur-xl">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl font-black text-slate-50 tracking-tight">Outstation Leads CRM</h1>
          </div>
          <p className="text-xs text-slate-400">
            Manage One-Way and Round-Trip intercity highway trip leads and assign outstation chauffeurs.
          </p>
        </div>
              ...
              {loading ? (
                <tr>
                  <td colSpan={7} className="p-8 text-center text-slate-500">
                    Loading Outstation leads...
                  </td>
                </tr>
              ) : leads.length === 0 ? (
                <tr>
                  <td colSpan={7} className="p-8 text-center text-slate-500">
                    No Outstation leads found.
                  </td>
                </tr>
              ) : (
                leads.map((lead) => (
                  <tr key={lead.id} className="hover:bg-slate-800/40 transition-colors">
                    <td className="p-4 font-bold text-slate-100">{lead.customerName}</td>
                    <td className="p-4 space-y-0.5">
                      <div className="font-mono text-slate-200">{lead.phone}</div>
                      <div className="text-[11px] text-slate-400">{lead.email}</div>
                    </td>
                    <td className="p-4 font-medium text-amber-300">{lead.tripType || "Outstation"}</td>
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-white/10 rounded-2xl p-6 max-w-2xl w-full space-y-6 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-start border-b border-white/10 pb-4">
              <div>
                <span className="text-[10px] font-mono font-bold text-amber-400 uppercase tracking-widest block">OUTSTATION LEAD DETAILS</span>
            ...
            <div className="grid grid-cols-2 gap-4 text-xs">
              ...
              <div className="bg-slate-950 p-3 rounded-xl border border-white/5">
                <span className="text-slate-500 uppercase tracking-wider block text-[10px]">Trip Type</span>
                <span className="font-bold text-amber-400 block mt-1">{activeLead.tripType || "Outstation"}</span>
            ...
            <div className="bg-slate-950 p-4 rounded-xl border border-white/5 space-y-2 text-xs">
              <div><strong>Pickup Address:</strong> {activeLead.pickupLocation}</div>
              <div><strong>Drop Address:</strong> {activeLead.dropLocation || "N/A"}</div>
            </div>
