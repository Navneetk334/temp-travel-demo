  ...
  return (
    <div className="flex justify-center w-full">
      <div className="space-y-2 relative w-full max-w-xl">
        <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider block h-4 leading-4 text-center">Company Name *</label>
            ...
            <div className="absolute left-0 right-0 top-full mt-1 max-h-64 overflow-y-auto bg-slate-900 border border-white/15 rounded-lg shadow-2xl z-50 py-1 divide-y divide-white/5 text-left">
              {value.trim().length > 0 && (
                ...
                >
                  ...
                  <span>Use entered company: <strong className="text-white font-bold">&quot;{value}&quot;</strong></span>
                </button>
              )}
              ...
              {googleSuggestions.length > 0 && (
                <div>
                  ...
                  {googleSuggestions.map((place, idx) => (
                    <button
                      key={idx}
                      type="button"
                      onMouseDown={() => {
                        onChange(place.mainText || place.fullText);
                        setShowSuggestions(false);
                      }}
              ...
              <div>
                ...
                {POPULAR_COMPANIES.filter(c => c.toLowerCase().includes((value || "").toLowerCase())).slice(0, 12).map(company => (
                  <button
                    key={company}
                    type="button"
                    onMouseDown={() => {
...
export default function BookingWidget() {
  ...
  // Active Company Query across tabs
  const activeCompanyQuery = (
    activeTab === "corporate"
      ? corpData.company
      : activeTab === "local"
        ? localData.company
        : outstationData.company
  ).trim();
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        const res = await fetch(`/api/places/autocomplete?q=${encodeURIComponent(activeCompanyQuery)}`);
        if (res.ok) {
            ...
            <div className="space-y-6">
              ...
              {pickupDropSubTab === "working" && (
                <CompanyNameSelector
                  ...
                  onChange={(val) => setCorpData({ ...corpData, company: val })}
