...
export async function GET(
  ...
  try {
    ...
    console.error(`GET /api/fleet/${params.id} error:`, error);
...
export async function PUT(
  ...
  try {
    ...
    console.error(`PUT /api/fleet/${params.id} error:`, error);
    return NextResponse.json({ error: "Internal Server Error" }, { status: 500 });
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    const bookingsCount = await prisma.booking.count({
      where: { vehicleId: params.id, status: { in: ["CONFIRMED", "DRIVER_ASSIGNED", "IN_TRANSIT"] } }
    });
    ...
    console.error(`DELETE /api/fleet/${params.id} error:`, error);
    return NextResponse.json({ error: "Internal Server Error" }, { status: 500 });
  }
}
